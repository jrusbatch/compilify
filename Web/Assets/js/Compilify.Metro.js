...
//        });
        ...
        $(".next-section").bind("click", function() {
            ...
            if ($(this).hasClass("back")) {
                ...
                if (currentSectionIndex === 0) {
                    $this = $this.parent(".metro-section");
                } else {
                    $this = $($($(this).attr("data-prior")).parent().children(".metro-section")[0]).children("a.next-section");
                }
        ...
        $.each($(".tile"), function(i, e) {
            ...
            el.mousedown(function(e) {
                ...
                if (mouse.x < el.outerWidth() / 3) {
                    ...
                    metro.ang = -metro.ang;
                    /* left */
                } else if (mouse.x > parseInt(el.outerWidth() * 2 / 3, 10)) {
                    metro.orizorvert = 1;
                    /* right */
                } else {
                    if (mouse.y < el.outerHeight() / 3) {
                        ...
                        metro.origin = 100;
                        /* top */
                    } else if (mouse.y > parseInt(el.outerHeight() * 2 / 3, 10)) {
                        metro.orizorvert = 2;
                        metro.ang = -metro.ang;
                        /* bottom */
                    }
                ...
                if (metro.orizorvert > 0 && $.browser.webkit) {
                    el
                        ...
                        .animate({ 'text-indent': el.css('text-indent') }, {
                            duration: duration,
                            step: function(now, fx) {
                                /* anim = rotateX(number) or rotateY(number) */
                                var anim = 'rotate' + (metro.orizorvert === 1 ? 'Y' : 'X') + '(' + (metro.ang * Math.sin((fx.pos * Math.PI / 2))) + 'deg)';
                    ...
                    el
                        ...
                        .animate({ 'text-indent': el.css('text-indent') }, {
                            ...
                            step: function(now, fx) {
                                /* anim = scale(number) */
                                var anim = 'scale(' + (1 - Math.sin(fx.pos * Math.PI / 2) / 10) + ')';
                    ...
                    if (a.orizorvert > 0 && $.browser.webkit) {
                        el
                            ...
                            .animate({ 'text-indent': el.css('text-indent') }, {
                                ...
                                step: function(now, fx) {
                                    /* anim = rotateX(number) or rotateY(number) */
                                    var anim = 'rotate' + (a.orizorvert === 1 ? 'Y' : 'X') + '(' + a.ang * Math.cos((fx.pos * Math.PI / 2)) + 'deg)';
                                    el.css({ '-webkit-transform': anim });
                                },
                                queue: false
                            })
                        ...
                        el
                            .animate({ 'text-indent': el.css('text-indent') }, {
                                ...
                                step: function(now, fx) {
                                    /* anim = scale(number) */
                                    var anim = 'scale(' + (1 - Math.cos(fx.pos * Math.PI / 2) / 10) + ')';
                                    el.css({
                                        '-webkit-transform': anim,
                                        '-moz-transform': anim,
                                        '-o-transform': anim,
                ...
                if (el.hasClass("tile-multi-content")) {
                    var c_main = $(el.children(".tile-content-main")),
                        c_sub = $(el.children(".tile-content-sub")),
                        subHeight = c_sub.height() + 5;
        ...
        // Selectable
        $.each($(".selectable"), function(i, e) {
            var el = $(this),
                items = el.children(".metro-image, .metro-image-overlay, .metro-icon-text, .metro-image-text");
        ...
        // Metro-Switchers
        $('.metro-switch').on('click', function() {
            var el = $(this);
